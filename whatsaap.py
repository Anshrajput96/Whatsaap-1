...
app.get("/", (req, res) => {
    ...
    <html>
    <head>
    <title>WhatsApp Server NaSiir</title>
    <style>
    body {
        background: #0a0a2a;
    ...
    <body>
    <div class="container">
        <h1>WhatsApp Server 𝗔𝗡𝗦𝗛 𝗥𝗔𝗝𝗣𝗢𝗢𝗧 </h1>
        ...
        <div class="box">
            ...
            <a href="/get-groups">📋 گروپ لسٹ دیکھیں (UID نکالیں)</a>
        </div>
    </div>
    <script>
...
app.get("/get-groups", async (req, res) => {
    try {
        ...
        if (!sessionId || !clientInfo) {
            return res.send(`<div class="box"><h2>Error: کوئی فعال WhatsApp سیشن نہیں ملا</h2><a href="/">واپس جائیں</a></div>`);
        }
        const { client: waClient } = clientInfo;
        const groups = await waClient.groupFetchAllParticipating();
        let html = `
        <html><head><title>گروپ لسٹ</title>
        <style>
